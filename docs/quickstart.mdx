...
We will be using [zot registry](https://zotregistry.io/) as our example.

If you would like to distribute to remote registries, 
you may refer to our [how-to guide](./how_to_guides/distributing_to_remote_registries.mdx).
...
In order to use any local registry, we need to first install it.
We will be running zot using docker,
but you can refer to their [installation guide](https://zotregistry.io/install-guides/install-guide-linux/) 
to find more ways to install the registry.
...
You may refer to [`oras push`](./commands/oras_push.mdx) for more details on which flags to use.
...
Let's now use [`oras pull`](./commands/oras_pull.mdx) to pull the artifact we have pushed in the previous step.
...
First, let's create another sample file to [attach](./commands/oras_attach.mdx) to the previously uploaded artifact. 
...
You can stop and remove the running quick start local zot registry and the uploaded content. 
...
As OCI registries are used to securely store and share container images, 
they greatly help with collaboration and code sharing. 
They enable teams to acquire and use images and artifacts through a standardized artifact interface. 
This is why it is considered to play a crucial role in maintaining consistency among teams.