...
```bash
oras attach ghcr.io/${GHCR_USER}/my-repository:v1 --artifact-type 'doc/example' --annotation 'key=val' sub-artifact.txt 
```

The output will look something like this:
```
✓ Uploaded  sub-artifact.txt                                                                                           13/13  B 100.00%  490ms
